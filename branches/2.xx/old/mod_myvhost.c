...
static void default_host(myvhost_cfg_t *cfg, core_server_config *scfg, request_rec *r)
{
	...
	ap_log_rerror(APLOG_MARK, APLOG_NOERRNO | APLOG_WARNING, r, "zend_restore_ini_entry(open_basedir) failed");
    }
    if (zend_restore_ini_entry("safe_mode", sizeof("safe_mode"), PHP_INI_STAGE_RUNTIME) < 0) {
	ap_log_rerror(APLOG_MARK, APLOG_NOERRNO | APLOG_WARNING, r, "zend_restore_ini_entry(safe_mode) failed");
    }
#endif
}
...
static int myvhost_translate(request_rec *r)
{
...
#ifdef WITH_CACHE
#ifdef WITH_PHP
	    cache_vhost_add(cfg, r->hostname, 0, 0, 0, -1);
#else
	    cache_vhost_add(cfg, r->hostname, 0, 0, -1);
#endif /* WITH_PHP */
#endif /* WITH_CACHE */
	}
	ap_unblock_alarms();
	...
	case 3:
	    if (row[2]) {
		php_ini_conf = ap_pstrdup(r->pool, row[2]);
	    }
#endif /* WITH_PHP */
	case 2:
	    if (row[1]) {
...
VHOST_FOUND:
	...
	if (!vhost) { /* not found in cache */
	    ...
	    {
#ifdef WITH_PHP
		cache_vhost_add(cfg, r->hostname, rootdir, admin, php_ini_conf, 1);
#else
		cache_vhost_add(cfg, r->hostname, rootdir, admin, 1);
#endif
	    }
	    ap_unblock_alarms();
#ifdef DEBUG
