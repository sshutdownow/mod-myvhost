...
struct ap_hash_t {
    ...
    ap_hash_entry_t    *free;  /* List of recycled entries */
};
...
static ap_hash_entry_t **alloc_array(ap_hash_t *ht, unsigned int max)
{
...
API_EXPORT(ap_hash_t *) ap_hash_make(ap_pool *pool)
{
    ...
    ht->hash_func = ap_hashfunc_default;
    return ht;
}
...
static void expand_array(ap_hash_t *ht)
{
    ap_hash_index_t *hi;
                                     ...
                                     const void *val)
{
    ...
    if ((he = ht->free) != NULL)
        ...
        he = ap_palloc(ht->pool, sizeof(*he));
    he->next = NULL;
    he->hash = hash;
    he->key  = key;
    he->klen = klen;
...
{
    ap_hash_t *ht;
    ap_hash_entry_t *new_vals;
    unsigned int i, j;

    ht = ap_palloc(pool, sizeof(ap_hash_t) +
                    sizeof(*ht->array) * (orig->max + 1) +
                    sizeof(ap_hash_entry_t) * orig->count);
    ht->pool = pool;
    ht->free = NULL;
    ht->count = orig->count;
    ht->max = orig->max;
    ht->hash_func = orig->hash_func;
    ht->array = (ap_hash_entry_t **)((char *)ht + sizeof(ap_hash_t));

    new_vals = (ap_hash_entry_t *)((char *)(ht) + sizeof(ap_hash_t) +
                                    sizeof(*ht->array) * (orig->max + 1));
    j = 0;
    for (i = 0; i <= ht->max; i++) {
        ap_hash_entry_t **new_entry = &(ht->array[i]);
        ap_hash_entry_t *orig_entry = orig->array[i];
        while (orig_entry) {
...
{
    ap_hash_t *res;
    ap_hash_entry_t *new_vals = NULL;
    ap_hash_entry_t *iter;
    ap_hash_entry_t *ent;
    unsigned int i,j,k;
     ...
     * as long as p
     */
    if (!ap_pool_is_ancestor(overlay->pool, p)) {
        fprintf(stderr,
                "ap_hash_merge: overlay's pool is not an ancestor of p\n");
        abort();
    }
    if (!ap_pool_is_ancestor(base->pool, p)) {
        fprintf(stderr,
                "ap_hash_merge: base's pool is not an ancestor of p\n");
        abort();
    }
#endif

    res = ap_palloc(p, sizeof(ap_hash_t));
    res->pool = p;
    res->free = NULL;
    ...
    if (base->count + overlay->count) {
        new_vals = ap_palloc(p, sizeof(ap_hash_entry_t) *
...
AP_POOL_IMPLEMENT_ACCESSOR(hash)
